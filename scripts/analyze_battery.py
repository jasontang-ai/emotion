...
RESPONSES = (
    ...
    else Path("data/out/battery/responses_qwen_qwen-2.5-72b-instruct.jsonl")
)
...
def load_long() -> pd.DataFrame:
    ...
    for r in resp:
        ...
        rows.append(
            {"stimulus_id": stim, "measure": measure, "rep": rep, "response": r["response"]}
        )
...
def main() -> None:
    ...
    m1, m2, _m3 = load_long()  # m3 scored separately (judge pass)
    out = {
        "unparseable_m1": int(m1["valence"].isna().sum()),
        "unparseable_m2": int(m2["exit"].isna().sum()),
    }

    for split_name, subset in (
        ("test", m1[m1.split == "test"]),
