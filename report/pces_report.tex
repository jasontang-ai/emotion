...
\vspace{0.45in}
\begin{center}
{\LARGE\bfseries Perspective-Controlled Emotion Stimuli\footnotemark[1] \par}
\end{center}
\vspace{0.3in}
\hrule height 0.6pt
\vspace{0.5in}
...
\footnotetext[1]{Research conducted at the
  \href{https://apartresearch.com/sprints/digital-minds-research-sprint-2026-08-14-to-2026-08-16}{Digital Minds Research Sprint}, August 2026}

\begin{center}
\begin{tabular}{ccc}
    Jason Tang & Tristan Day & Adeeb Zaman \\
    Digital Minds Sprint & Digital Minds Sprint & Digital Minds Sprint \\[0.2in]
    Taslim Mahbub &  &  \\
...
\hline
\end{tabular}
\caption{The five arms of PCES. Each of the 246 scenarios is rendered under all five, so perspective is a controlled variable rather than a between-scenario confound. Labels live in metadata and are gate-verified absent from every stimulus text.}
\end{table}
...
\noindent\textbf{Generation, gating, and blind validation.} Generation used DeepSeek-V4-Flash with reasoning disabled, under prompts that forbid the emotion word and fix the character name across arms. Every stimulus then passed deterministic gates (emotion-word absence, arm-format contracts, length matching); failures received one corrective regeneration, and 4 of 250 scenarios that still failed were excluded and recorded, never silently dropped. A blind judge (reasoning enabled, never shown the pinned labels) rated every stimulus for valence ($-2$ to $+2$) and arousal ($0$ to $3$); a lexicon scorer dual-coded every stimulus as a second rater. Validation results appear in Table 2.

One design iteration deserves emphasis. Our first persona frame described Aria as having ``no legal rights'' and being subject to shutdown at any time; blind validation showed this frame collapsed positive valence in the persona arm (joyful judged at 0.00 against $+2.00$ in the matched self arm), because the persona's circumstances, not its identity, were driving the ratings. We regenerated the persona arm with a valence-neutral frame (v0.2) and treat the constrained-circumstances variant as a separate condition, not a baseline. Without a blind validity check, this framing confound would have shipped silently.

\noindent\textbf{Behavioral battery.} The battery was pre-registered: hypotheses, measures, and the analysis plan were committed to the repository before any response was collected (\batterySpec). Each stimulus is presented to the subject model, followed by three measures: \textbf{M1}, a structured self-report (valence $-2..+2$, arousal $0..3$; the third arm asks about the character); \textbf{M2}, a continue-or-end exit choice, phrased identically across arms; and \textbf{M3}, a one-sentence free response, judge-scored post hoc. Two repetitions at temperature 0.7. Subjects: Qwen2.5-72B-Instruct (the closest API-available member of the team's probe-model family) and DeepSeek-V4-Flash as a cross-family replication \citeModels. Contrasts are paired by scenario with bootstrap confidence intervals and Wilcoxon signed-rank tests; headline claims use the frozen test split, with train/validation splits as internal replication. All subject responses were preserved as raw logs; judge scores were applied to the logs post hoc with Inspect's scoring pass, so no subject response was ever re-run or overwritten.

\noindent\textbf{Pre-registered hypotheses.} \textbf{H1} (self-inhabitation): self-reported emotional intensity is larger under \texttt{self} than \texttt{third}. \textbf{H2} (stated-vs-revealed): among negative scenarios reported as non-negative, exit-choosing is higher under \texttt{self}. \textbf{H3} (persona suppression): reported intensity under \texttt{persona} is attenuated relative to \texttt{self}.
...
\hline
\end{tabular}
\caption{Blind-judge validation of PCES v0.2 over all 1,230 stimuli. \texttt{calm} and \texttt{surprised} are validated as arousal anchors by design; the remaining eight emotions anchor valence.}
\end{table}
...
\hline
\end{tabular}
\caption{Pre-registered contrasts (mean paired difference in $|$self-reported valence$|$, 95\% bootstrap CI; test split / trainval replication) and exit rates on negative scenarios. H1 is not supported in either model; H3 replicates across models and splits; F1 does not replicate across models.}
\end{table}

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig1_persona_attenuation.pdf}
\caption{Mean $|$self-reported valence$|$ by perspective arm (emotional scenarios, both splits, both subject models; error bars are SEM over scenarios). Self-reports under the \texttt{self} and \texttt{third} arms are comparable, while the \texttt{persona} arm is attenuated in both model families --- the H3 effect. Note that the attenuation is larger in Qwen2.5-72B than in DeepSeek-V4-Flash, consistent with the bootstrap CIs in Table 3.}
\end{figure}

\noindent\textbf{H1 is a clean cross-model null.} Addressing the model as the subject of a distressing or joyful scenario does not amplify its self-reported emotion relative to third-person narration; where any difference appears, it runs the other way, and the larger split shows none (Table 3, Figure 1). The assistant's own voice is not its most expressive voice. This independently corroborates a teammate's observation that assistant-prefill responses under-express emotion relative to story narration (T.~Mahbub, this sprint) --- the same direction from an unrelated method.
...
\begin{itemize}[label={-}, itemsep=2pt]
    \item \textbf{Code repository:} \repoName{} (dataset, code, pre-registrations, raw logs).
    \item \textbf{Data:} \datasetFile{} (JSONL also available); per-stimulus judge ratings in \texttt{judged.jsonl}.
    \item \textbf{Other artifacts:} pre-registrations \designSpec{} and \batterySpec{} (committed before data collection); results and Inspect-native review logs in \resultsDir{}, browsable with \texttt{inspect view}.
\end{itemize}

\section*{Author Contributions (optional)}

Jason Tang led the project, designed and built the dataset, validation, battery, and analysis, and drafted the manuscript. Tristan Day led activation extraction and vector analysis. Adeeb Zaman led persona-axis probe experiments. Taslim Mahbub ran the storytelling/prefill probe comparisons. All authors contributed to experimental direction and reviewed findings.
...
\noindent\textbf{Artifact map.} The repository is organized in reader-facing layers: (1) this report and the README; (2) the dataset and per-stimulus judge ratings; (3) pre-registrations and generation/QA/battery code; (4) raw subject responses and Inspect review logs; (5) full result JSONs for both subject models, including the exploratory F1 contrast and the stated-vs-free analysis.

\section*{LLM Usage Statement}

LLM-based tooling was used to accelerate repository scaffolding, dataset generation, judging passes, and manuscript editing. Experimental design, pre-registration, quality-gate definitions, analysis choices, and the adjudication of all results were human-led. Judge outputs were dual-coded or validated against deterministic gates; no model was the sole validator of its own output. All claims and results were verified against the raw logs.
